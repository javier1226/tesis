...
class Usuario{
    ...
    function editar($id_usuario,$celular,$residencia,$correo,$sexo,$adicional){
        $sql="UPDATE usuario SET celular_us=:celular,residencia_us=:residencia,correo_us=:correo,sexo_us=:sexo,adicional_us=:adicional where id_usuario=:id";
        $query = $this->acceso->prepare($sql);
        $query->execute(array(':id'=>$id_usuario,':celular'=>$celular,':residencia'=>$residencia,':correo'=>$correo,':sexo'=>$sexo,':adicional'=>$adicional));
    }
